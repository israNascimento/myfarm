...
import { Item,Card,List,ListItem,View,Input,Icon, Button, Text, Toast,Header,Left,Right,Body,Content,Container,Form } from "native-base";
import { NavigationActions } from 'react-navigation';

const analytics = require('./instancias/analytics');
const {bancoLocal,registraHistorico} = require('./instancias/conexao.js');
...
var form1 = [
    ...
    {nome: "quantidade",placeholder: "Quantidade (ml ou g)",tipo: "numeric"},
    {nome: "preco",placeholder: "Preço estimado (R$)",tipo: "numeric"},
    {nome: "coadjuvante",placeholder: "Produto (Coadjuvante)"},
    {nome: "c_quantidade",placeholder: "Quantidade (ml ou g) (Coadjuvante)",tipo: "numeric"},
    {nome: "volume",placeholder: "Volume de calda (litros)",tipo: "numeric"},
    ...
    return(
        <Container style={{paddingTop: Platform.OS === 'ios' ? 0 : StatusBar.currentHeight}}>
          <Header>
              ...
              <Body><Text style={{color:Platform.OS === 'ios' ? "black" : "white",width:200}}>Cadastro de Aplicações</Text></Body>
              <Right />
          </Header>
          ...
          <Content style={{backgroundColor:'#eee',padding:15}}>
              ...
              <Text style={{fontSize:14,color:'#444', marginBottom:8,marginLeft:5}}>Registro da aplicação de acaricidas, inseticidas e nematicidas.</Text>
              ...
              <Button block style={{marginTop: 15,marginBottom:25}} onPress={()=>{
                  ...
                  let data = new Date();
                  registraHistorico("aplicacoes",{time: data.getDate() + "/" + (data.getMonth() + 1) + "/" + data.getYear(), title: "Aplicação de " + tmp.produto, description: tmp.quantidade + " ml"});
