...
import { Item,View,Input,Icon, Button, Text, DatePicker,Toast } from "native-base";
...
export class Texto extends React.Component {
    render(){
        if(this.props.tipo != "data"){
            return(
                <Item style={{marginBottom: 12,flex:1,minHeight:this.props.tamanho}}>
                    <Icon active name={this.props.icone} style={{color:this.props.cor}}/>
                    <Input style={{height: this.props.tamanho,color: this.props.cor}} keyboardType={this.props.tipo} placeholder={this.props.placeholder} placeholderTextColor={this.props.corP} returnKeyType={"next"} secureTextEntry={this.props.senha} {...this.props}>
                        {this.props.texto}
                    </Input>
                </Item>
            )
...
export class Formulario extends React.Component {
    ...
    verificaRequisitos = () => {
        ...
        for(var i in this.state.campos){
            if(this.state.campos[i]["obrigatorio"] && this.state[this.state.campos[i]["nome"]] == ""){
                erro++;
