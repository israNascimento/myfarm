...
export default class Principal extends React.Component {
  ...
  componentWillMount () { analytics.trackScreenView("Login"); this.keyboardDidShowListener = Keyboard.addListener('keyboardDidShow', this._keyboardDidShow); this.keyboardDidHideListener = Keyboard.addListener('keyboardDidHide', this._keyboardDidHide); }
  componentWillUnmount () { this.keyboardDidShowListener.remove(); this.keyboardDidHideListener.remove(); }
  _keyboardDidShow = () => { this.setState({ teclado: true }); }
  _keyboardDidHide = () => { this.setState({ teclado: false }); }
  //=======================================//
  ...
  render(){
      ...
      if(!this.state.teclado){
          logo = <Image source={require('./assets/logo2.png')} style={styles.logo} />
      }

      return(
          <ImageBackground source={require('./assets/1.jpg')} style={styles.imageContainer}>
              <Icon active name="ios-information-circle" style={{color:"#fff",fontSize:26,position:"absolute",zIndex:999,right: 15,top:35}} onPress={() => navigate('Sobre')}/>
