    ...
    render(){
        return(
            <Item style={{marginBottom: 12}}>
                ...
                <Input style={{height: 60,color: "#ffffff"}} placeholder={this.props.placeholder} placeholderTextColor="#ffffff" returnKeyType={"next"} {...this.props}>
                    {this.props.texto}
                </Input>
            </Item>
        );
...
var formularios = [[
    {nome: "nome_proprietario",placeholder: "Nome do proprietário",icone:"person"},
    {nome: "telefone",placeholder: "Telefone",icone:"ios-call",tipo:"numeric"},
    ...
    {nome: "bairro",placeholder: "Bairro",icone:"md-locate",tipo:"numeric"},
    {nome: "logradouro",placeholder: "Logradouro",icone:"md-compass"},
    {nome: "complemento",placeholder: "Complemento",icone:"ios-information-circle"}
]];
...
render = () => {
  ...
  return(
    <ImageBackground blurRadius={8} source={require('./assets/1.jpg')} style={{flex:1, width: null,padding: 20,paddingTop: 20}}>
        <View style={{alignItems:'center'}}>
            <Text style={{color: '#fff',backgroundColor:'transparent',fontSize: 32, marginTop: 10,marginBottom: 35,textShadowColor :'#050',textShadowOffset:{width: 0,height:1},textShadowRadius:5}}>Cadastro</Text>
        </View>
        ...
        <View style={{flex:1}}>
            <ScrollView style={this.state.posicao == 0 ? {} : {height:0,opacity:0}}>
                <Formulario style={{paddingHorizontal:0}} tamanho={60} campos={formularios[0]} cor="#fff" corP="#fff" ref={(tmp) => {this._formulario1 = tmp}}/>
            </ScrollView>

            <ScrollView style={this.state.posicao == 1 ? {} : {height:0,opacity:0}}>
                <Texto placeholder="CEP" icone="ios-pin" keyboardType='numeric' maxLength={8} value={this.state.cep}
                  onChangeText={(txt) => {
                ...
                <Formulario style={{paddingHorizontal:0}} tamanho={60} campos={formularios[1]} cor="#fff" corP="#fff" ref={(tmp) => {this._formulario2 = tmp}}/>
            </ScrollView>

            <ScrollView style={this.state.posicao == 2 ? {} : {height:0,opacity:0}}>
                <Texto placeholder="Nome da propriedade" icone="home" onChangeText={(txt) =>{
                    this.setState({formulario: Object.assign(this.state.formulario,{"nome_propriedade":txt})});
                }} />
                <View>
                    <MapView
                      ref="mapa"
                    ...
                    <Button success style={{position:'absolute',top:5,right:5}}
                    onPress={() => {
                        var tmp = ""; if(this.state.mapaAtivo) tmp = "md-checkmark"; else tmp = "md-create";
                        this.setState({mapaAtivo:!this.state.mapaAtivo,btnMapa:tmp});
                    }}>
        ...
        <HideWithKeyboard><View style={{flexDirection:'row'}}>
            <Button rounded danger style={{ margin: 15 }} onPress={() => {
                  if(this.state.posicao == 0) this.props.navigation.dispatch(NavigationActions.back());
                  else this.setState({posicao:--this.state.posicao});
                }}>
                <Icon name="arrow-back" />
            </Button>

            <Button rounded style={{ margin: 15,flex:1,backgroundColor:'#004238' }} onPress={() => {
                if(this.state.posicao == 2){
                    if(!this.state.editando) { mostraToast("Defina a posição da propriedade no mapa"); return; }

                    // console.warn(Object.assign(this.state.formulario,this.state.editando.coordenadas))

                    bancoLocal.put(Object.assign({"_id":"dados"},this.state.formulario,{"coordenadas":this.state.editando.coordenadas}), function(erro,doc){
                ...
                if(this.state.posicao == 1){
                    ...
                    this.setState({
                        formulario: Object.assign(
                            ...
                            {"cep": this.state.cep,"uf": this.state.endereco.uf},
                            {"propriedades":tmp}
                        ), posicao:++this.state.posicao
                    })
                }
                if(this.state.posicao == 0){
                    this.setState({formulario: Object.assign(this.state.formulario,this._formulario1.getValores()),posicao:++this.state.posicao})
                }
