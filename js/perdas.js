...
var perdas = {}
bancoLocal.get('perdas', function(erro,doc){
    if(erro) {
        perdas = {
            "_id" : "perdas"
        }
...
function Registro(props){
    return(
        <ListItem style={{marginLeft:0,paddingRight:0,paddingTop:0,paddingBottom:0}}>
            <View style={{flexDirection:"row"}}>

                <View style={{paddingVertical:10,paddingHorizontal:15,alignItems:'center',justifyContent:'center'}}>
                ...
                <View style={{flex:1,paddingVertical:10,marginLeft: 20}}>
                  ...
                  <View style={{flexDirection:"row",marginTop:5}}>
                    ...
                    <Slider style={{width:200}} maximumValue={10} minimumValue={1} step={1} value={props.valor}
                      onValueChange={v => {
                      ...
                      onSlidingComplete={v =>{
                          bancoLocal.put(perdas, function(erro,doc){
                              if(erro) return;
                ...
                <View style={{paddingVertical:10,paddingHorizontal:15,alignItems:'center',justifyContent:'center'}}>
                    <Icon name="md-trash" style={{color:"#C62828"}} onPress={() => {
                        for(var tmp in perdas){
                            if(tmp == "_id" || tmp == "_rev") continue;
                            for(var i=0;i<perdas[tmp].length;i++){
...
function Lista(props){
    if(props.dados == undefined || props.dados.length == 0) return(
    ...
    return(
        <FlatList
            data={props.dados}
            renderItem={({ item }) => (
                <Registro nome={item.nome} valor={item.valor} id={item.id} handler = {props.atualiza}/>
            )}
...
export default class Perdas extends React.Component {
  ...
  atualiza(e){ this.setState({flag: !this.state.flag}); }

  componentWillMount(){ analytics.trackScreenView("Cadastro de Perdas"); }
  ...
  insere(val){
      ...
      perdas[secao[val.tipo]].push({"id":val.key,"nome":val.label,"valor":1,"tipo":val.tipo});
    ...
    return(
        <Container style={{paddingTop: Platform.OS === 'ios' ? 0 : StatusBar.currentHeight}}>
            <Header>
                ...
                <Body><Text style={{color:Platform.OS === 'ios' ? "black" : "white"}}>Perdas</Text></Body>
                <Right />
            </Header>
            ...
            <Content style={{backgroundColor:'#fff'}}>
              <List>
                  ...
                  <Lista dados={perdas.anomalias} atualiza={this.atualiza} flag={this.state.flag} />

                  <Item texto="Doenças fúngicas" itemDivider />
                  <Lista dados={perdas.fungicas} atualiza={this.atualiza} flag={this.state.flag} />

                  <Item texto="Bacterioses" itemDivider />
                  <Lista dados={perdas.bacterioses} atualiza={this.atualiza} flag={this.state.flag} />

                  <Item texto="Viroses" itemDivider />
                  <Lista dados={perdas.viroses} atualiza={this.atualiza} flag={this.state.flag} />

                  <Item texto="Pragas" itemDivider />
                  <Lista dados={perdas.pragas} atualiza={this.atualiza} flag={this.state.flag} />

                  <Item texto="Climáticas" itemDivider />
                  <Lista dados={perdas.climaticas} atualiza={this.atualiza} flag={this.state.flag} />
              </List>
          </Content>
