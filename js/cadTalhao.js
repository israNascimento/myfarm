...
import { Item,Card,List,ListItem,View,Input,Icon, Button, Text, Toast,Header,Left,Right,Body,Content,Container } from "native-base";
...
var form2 = [
    ...
    {nome: "espaco",placeholder: "Espaço (m) entre fileiras",tipo: "numeric"},
    {nome: "irrigacao",placeholder: "Irrigação"},
    {nome: "topografia",placeholder: "Topografia"}
];

var talhoes = {};
bancoLocal.get('talhoes', function(erro,doc){
    if(erro) {
        talhoes = {
            "_id" : "talhoes",
            "talhoes": []
    ...
    return(
        <Container style={{paddingTop: Platform.OS === 'ios' ? 0 : StatusBar.currentHeight}}>
          ...
          <Content style={{backgroundColor:'#eee',padding:15}}>
              ...
              <Text style={{fontSize:14,color:'#444', marginBottom:8,marginLeft:5}}>Informações sobre o talhão</Text>

              <Card style={{borderRadius:5,padding: 10}}>
                  <Formulario tamanho={40} campos={form1} cor="#000" corP="#555" ref={tmp => this._formulario1 = tmp}/>
              </Card>

              <Text style={{fontSize:18,marginLeft:5,marginTop:20}}>PLANTAÇÃO</Text>
              <Text style={{fontSize:14,color:'#444', marginBottom:8,marginLeft:5}}>Informações sobre a plantação</Text>

              <Card style={{borderRadius:5,padding: 10}}>
                  <Formulario tamanho={40} campos={form2} cor="#000" corP="#555" ref={tmp => this._formulario2 = tmp}/>
              </Card>

              <Text style={{fontSize:18,marginLeft:5,marginTop:20}}>MAPA</Text>
              <Text style={{fontSize:14,color:'#444', marginBottom:8,marginLeft:5}}>Informe a posição do talhão no mapa</Text>
              ...
              <Button block style={{marginTop: 15,marginBottom:25}} onPress={()=>{
                  var tmp = Object.assign(this._formulario1.getValores(),this._formulario2.getValores());
                  console.warn(tmp);

                  talhoes["talhoes"].push(tmp);
                  bancoLocal.put(talhoes, function(erro,doc){
                      if(erro) return;
                  });

                  this.props.navigation.state.params.anterior.setState({talhoes: talhoes["talhoes"]});
